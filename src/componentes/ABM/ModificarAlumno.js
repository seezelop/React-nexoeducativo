...
const ModificarAlumno = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.put(`http://localhost:8080/api/alumno/modificar/${formData.id_usuario}`, formData, { withCredentials: true });
  ...
  return (
    <Form onSubmit={handleSubmit}>
      ...
      <Form.Group controlId="id_usuario">
        ...
        <Form.Control type="text" value={formData.id_usuario} onChange={handleInputChange} required />
      </Form.Group>
